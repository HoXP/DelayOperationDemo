...
//  这个函数的意思就是每1秒钟执行一次SomeMethod,总共执行5次

//3.vp_Timer.In(1.0f, SomeMethod, 5, 2f);
//  1秒之后开始执行，每2秒钟执行一次，总共执行5次

//4.如果第一次希望立即执行，首参数传0
//  vp_Timer.In(0, SomeMethod, 5, 2f);

//5.传参数,使用object传参数，支持无限传参
...
//10.取消所有使用"SomeMethod"的Timer
//  vp_Timer.CancelAll("SomeMethod");

//11.永久执行一个Timer，对于这种Timer需要程序员自己来维护它的生命周期
//  private vp_Timer.Handle m_RaceTimer = new vp_Timer.Handle();
//  void BeginRace()
//  {
//      vp_Timer.Start(m_RaceTimer);
//  }
//  如果想取消m_RaceTimer.Cancel();

//如果需要自己维护Timer的生命周期，可以自己声明一个vp_Timer.Handle变量
//  private vp_Timer.Handle m_Timer = new vp_Timer.Handle();
//  vp_Timer.In(3.0f, SomeMethod, m_Timer);
...
public class vp_Timer : MonoBehaviour
{
	...
	private static float mMaxTime = 60 * 60 * 24 * 365;
	...
	public delegate void FinalCallback();
	...
	private void Update()
	{
		...
		while ((vp_Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
		{
			...
			vp_Timer.m_Active[m_EventIterator].WillLifeTime -= Time.deltaTime;
			...
			if (Time.time >= vp_Timer.m_Active[m_EventIterator].DueTime ||	// time is up
				...
				|| vp_Timer.m_Active[m_EventIterator].WillLifeTime < 0)					// event has been canceled ('Execute' will kill it)
	...
	/// delay的单位是秒,object arguments这个参数可以传进去object[],这样可以传递多个参数
	/// </summary>

	// time + callback + [timer handle]
	public static void In(float delay, Callback callback, Handle timerHandle = null)
	{ Schedule(delay, callback, null, null, null, timerHandle, 1, -1.0f, mMaxTime); }
	...
	{ Schedule(delay, callback, null, null, null, timerHandle, iterations, -1.0f, mMaxTime); }
	...
	{ Schedule(delay, callback, null, null, null, timerHandle, iterations, interval, mMaxTime); }
	...
	{ Schedule(delay, null, callback, arguments, null, timerHandle, 1, -1.0f, mMaxTime); }
	...
	{ Schedule(delay, null, callback, arguments, null, timerHandle, iterations, -1.0f, mMaxTime); }
	...
	{ Schedule(delay, null, callback, arguments, null, timerHandle, iterations, interval, mMaxTime); }
	...
	{
		Schedule(315360000.0f, /* ten years, yo ;) */ delegate(float deltaTime) { }, null, null, null, timerHandle, 1, -1.0f, 0);
	}
	...
	/// certain method
	/// </summary>
	public static bool CancelAll(string methodName)
	{
		bool isCancel = false;
		for (int t = vp_Timer.m_Active.Count - 1; t > -1; t--)
		{
	...
	public static bool Cancel(Callback fun)
	{
		if (fun == null)
		{
			return false;
		...
		for (int t = vp_Timer.m_Active.Count - 1; t > -1; t--)
		{
			if (vp_Timer.m_Active[t].Function == fun)
			{
				vp_Timer.m_Active[t].Id = 0;
				return true;
			}
	...
	private class Event
	{
		...
		public FinalCallback FinalFunction = null;
		// 执行次数
		public int Iterations = 1;
		// 间隔时间
		public float Interval = -1.0f;
		// 消耗时间
		public float DueTime = -1.0f;
		// 开始时间
		public float StartTime = 0.0f;
		//持续时间
		public float WillLifeTime = 0.0f;
		//
		...
		public void Execute()
		{
			...
			if (FinalFunction != null && WillLifeTime < 0)
			{
				FinalFunction();
				Recycle();
				return;
