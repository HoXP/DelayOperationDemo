...
public class vp_Timer_UnScaleTime : MonoBehaviour
{
	...
	private void Update()
	{
		...
		m_EventBatch = 0;
		while ((vp_Timer_UnScaleTime.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
		{
			//Debug.LogError("Update()>>111");
			...
			if (m_EventIterator < 0)
			{
				...
				m_EventIterator = vp_Timer_UnScaleTime.m_Active.Count - 1;
				break;
			}
			// prevent index out of bounds
			if (m_EventIterator > vp_Timer_UnScaleTime.m_Active.Count - 1)
				m_EventIterator = vp_Timer_UnScaleTime.m_Active.Count - 1;

			// execute all due events
			Event eEvent = vp_Timer_UnScaleTime.m_Active[m_EventIterator];

			//if (eEvent.Id == TRACEID)
			//{
			//    Debug.LogError("Update()>>Time.time" + Time.time + "DueTime" + eEvent.DueTime + "  ID" + eEvent.Id);
			//}
			if (Time.unscaledTime >= vp_Timer_UnScaleTime.m_Active[m_EventIterator].DueTime || // time is up
				vp_Timer_UnScaleTime.m_Active[m_EventIterator].Id == 0)
			{
				//Debug.LogError("Update()>>2222" + TRACEID + " " + eEvent.Id);
				//if (eEvent.Id == TRACEID)
				//{
				//    Debug.LogError("Update()>>2");
				//}
				vp_Timer_UnScaleTime.m_Active[m_EventIterator].Execute();
			...
			else
			{
				...
				if (vp_Timer_UnScaleTime.m_Active[m_EventIterator].Paused)
					vp_Timer_UnScaleTime.m_Active[m_EventIterator].DueTime += Time.unscaledDeltaTime;
				else
					// log lifetime
					vp_Timer_UnScaleTime.m_Active[m_EventIterator].LifeTime += Time.unscaledDeltaTime;
			}

			// going backwards since 'Execute' will remove items from the list
			m_EventIterator--;
			m_EventBatch++;
			//  Debug.LogError("Update()>>4444");
		}
	}
	...
	/// </summary>
	private static void Cancel(vp_Timer_UnScaleTime.Handle handle)
	{
		if (handle == null)
			return;
	...
	{
		for (int t = vp_Timer_UnScaleTime.m_Active.Count - 1; t > -1; t--)
		{
			vp_Timer_UnScaleTime.m_Active[t].Id = 0;
		}
	}
	...
	public static void CancelAll(string methodName)
	{
		for (int t = vp_Timer_UnScaleTime.m_Active.Count - 1; t > -1; t--)
		{
			if (vp_Timer_UnScaleTime.m_Active[t].MethodName == methodName)
				vp_Timer_UnScaleTime.m_Active[t].Id = 0;
		}
	}
	...
	{
		vp_Timer_UnScaleTime.m_Active.Clear();
		vp_Timer_UnScaleTime.m_Pool.Clear();
	...
	private class Event
	{
		...
		public void Execute()
		{
			...
			if (Function != null)
			{
				//   Debug.LogError("开始 执行FunctionFunctionFunctionFunction");
				Function();
			}

			else if (ArgFunction != null)
			{
				// Debug.LogError("开始 执行ArgFunctionArgFunctionArgFunctionArgFunction");
			...
			else
			{
				...
				// Debug.LogError("Execute 开始执行 》》》》》》》》》》》》》》1111》》》》》》Recycle();");
				Recycle();
				return;
			}
			...
			if (Iterations > 0)
			{
				...
				if (Iterations < 1)
				{
					//Debug.LogError("Execute 开始执行 》》》》》》》》》》》》》》2222》》》》》》Recycle();");
					Recycle();
					return;
				}
			}
			...
			DueTime = Time.unscaledTime + Interval;
		...
		private void Destroy()
		{
			...
			vp_Timer_UnScaleTime.m_Active.Remove(this);
			vp_Timer_UnScaleTime.m_Pool.Remove(this);
	...
	public class Handle
	{
		...
		private vp_Timer_UnScaleTime.Event m_Event = null;	// timer we're pointing at
		...
		public float TimeOfLastIteration
		{
			...
			{
				if (Active)
					return Time.unscaledTime + DurationLeft;
				return 0.0f;
			}
		}
		...
		public float TimeUntilNextIteration
		{
			...
			{
				if (Active)
					return m_Event.DueTime - Time.unscaledTime;
				return 0.0f;
			}
		}
		...
		public int Id
		{
			...
			set
			{
				...
				if (m_Id == 0)
				{
					...
					//  Debug.LogError("我在这里回收了>>> m_Event.Recycle(); m_Event.Recycle();");
					m_Event.Recycle();
					return;
				}
				...
				for (int t = vp_Timer_UnScaleTime.m_Active.Count - 1; t > -1; t--)
				{
					if (vp_Timer_UnScaleTime.m_Active[t].Id == m_Id)
					{
						m_Event = vp_Timer_UnScaleTime.m_Active[t];
						break;
					}
				}
				if (m_Event == null)
		...
		public void Cancel()
		{
			vp_Timer_UnScaleTime.Cancel(this);
		}
		...
		public void Execute()
		{
			m_Event.DueTime = Time.unscaledTime;
		}
	}
}